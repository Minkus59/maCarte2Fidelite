...
if (isset($_POST['Enregistrer3'])) {
    ...
    if (empty($Message)) {
        ...
        $UpdateParam=$cnx->prepare("UPDATE ".$Prefix."Fidelite_Param SET message=:message WHERE client=:client");
        $UpdateParam->bindParam(':message', $Message, PDO::PARAM_STR);
        $UpdateParam->bindParam(':client', $SessionCompteClient, PDO::PARAM_STR);
        $UpdateParam->execute();

        $Valid="Paramètre modifié avec succès";
        header("location:".$Home."/DashBoard/Compte/Parametre/Email/?valid=".urlencode($Valid));
    }
}
...
<div id="Form_Middle5">
<H1 class="TitreOrange">Email Type</H1>

<form name="SelectMode" action="" method="POST">
Message envoyé avec le chèque de fidélité <font color='#FF0000'>*</font> 

<textarea id="message" name="message" placeholder="Message*" require="required"><?php echo $Param->message; ?></textarea>
<BR />
<span class="col_1"></span>
<input type="submit" class="ButtonOrange" name="Enregistrer3" value="Enregistrer"/>
</form>
</div>
