...
if ((!empty($_GET['id']))&&(isset($_POST['oui']))) {
    ...
    $delete=$cnx->prepare("DELETE FROM ".$Prefix."Boutique WHERE id=:id AND client=:client");
    $delete->bindParam(':client', $SessionCompteClient, PDO::PARAM_STR);
    $delete->bindParam(':id', $_GET['id'], PDO::PARAM_STR);
    $delete->execute();

    header('Location:'.$Home.'/DashBoard/Compte/Multi-boutique/Boutique/');
...
?>
Etes-vous sur de vouloir supprimer cette boutique ?
