...
if ((!empty($_GET['id']))&&(isset($_POST['oui']))) {
    ...
    if (($Fideliter->fidelite!=2)&&($BonFideliter->consomed >= $Now)) {
        $Update=$cnx->prepare("UPDATE ".$Prefix."Historique SET fidelite='3' WHERE id=:id AND client=:client");
        $Update->bindParam(':id', $Id, PDO::PARAM_INT);
        $Update->bindParam(':client', $SessionCompteClient, PDO::PARAM_STR);
        $Update->execute();
        
        $Update=$cnx->prepare("UPDATE ".$Prefix."Bon_Fidelite SET fidelite='3' WHERE hash_transac=:hash AND hash_client=:hash_client AND client=:client");
        ...
        if (!empty($Retour)) {
            header('Location:'.$Home.'/DashBoard/Fidelite/Gestion/Historique/?id='.$Retour);
        }
        else {
            header('Location:'.$Home.'/DashBoard/Fidelite/Gestion/Historique/');
        }
    }
    else {
        $Erreur="Impossible de désactiver le bon d'achat<BR />";
        $Erreur.="La date de validité est dépassé ou le bon a déja été consommer";
        
        if (!empty($Retour)) {
            header('Location:'.$Home.'/DashBoard/Fidelite/Gestion/Historique/?id='.$Retour."&erreur=".urlencode($Erreur));
        }
        else {
            header('Location:'.$Home.'/DashBoard/Fidelite/Gestion/Historique/');
        }
...
Etes-vous sur de vouloir désactiver ce bon d'achat ? </p>
