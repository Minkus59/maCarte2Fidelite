...
try {
    ...
    $InsertPaiement=$cnx->prepare("INSERT INTO ".$Prefix."paiement (etat, id_transaction, montant, email, nom, prenom, commande, hash_client, origin, type) VALUES (:etat, :id_transaction, :montant, :email, :nom, :prenom, :commande, :hash_client, :origin, :type)");
    ...
    $SelectCompte=$cnx->prepare("SELECT * FROM ".$Prefix."compte WHERE hash=:hash");
 ...
 if ($Compte->actif=="0") {
        if ($ipn->customData==1) {
            if ($Montant=="75") {
                $UpdateAbo=$cnx->prepare('UPDATE '.$Prefix.'compte SET debut=:debut, fin=:fin, actif="2", abo=:abo WHERE hash=:hash');
 ...
 if ($Compte->actif=="2") {
        if ($ipn->customData==1) {
            if ($Montant=="75") {
                $UpdateAbo=$cnx->prepare('UPDATE '.$Prefix.'compte SET fin=:fin, abo=:abo WHERE hash=:hash');
 ...
 if ($Compte->actif=="3") {
        if ($ipn->customData==1) {
            if ($Montant=="75") {
                $UpdateAbo=$cnx->prepare('UPDATE '.$Prefix.'compte SET abo=:abo WHERE hash=:hash');
                //$UpdateAbo->bindParam(':fin', $TempsPre+$Duree1, PDO::PARAM_STR);
                $UpdateAbo->bindParam(':abo', $Abo1, PDO::PARAM_STR);
                $UpdateAbo->bindParam(':hash', $ipn->customer, PDO::PARAM_STR);
                $UpdateAbo->execute();   
